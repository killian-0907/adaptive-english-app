...
export type Task = { key: string; knowledgeId?: string; topic: string; prompt: string; context: string; options: string[]; answer: string | null; model: string; explanation: string; native: Record<string, string>; skill: Dimension; modality: Modality; strategy: "exact" | "evaluator" | "local_bounded"; audio: boolean; spoken: boolean; variant: number; criteria?: BoundedCriteria; scenario?: ScenarioProgress };
...
export type History = { id: string; session_id: string; status: string; created_at: string; metadata: { decision: Decision; task: Task; support?: number; replay?: number; transcript?: boolean; quality?: number | null; feedback?: string; correction?: string; elapsedMs?: number | null; skipped?: boolean } };
export const evidenceSchema = z.object({
  ...
  metadata: z.object({ difficulty: z.number().int().min(0).max(5), method: z.enum(methods), topic: z.string(), taskKey: z.string(), errors: z.array(z.string()), sessionState: z.string(), misunderstood: z.boolean(), firstListen: z.boolean(), retries: z.number().int().min(0), timed: z.boolean(), evaluationStrategy: z.string().optional(), voiceProvider: z.string().optional(), scenarioFamily: z.string().optional(), acousticUncertainty: z.boolean().optional(), skipped: z.boolean().optional() }).strict(),
}).strict();
