...
export const evidenceContext = (e: Evidence) => e.metadata.taskKey.startsWith("scenario:") ? e.metadata.taskKey.replace(/:\d+$/, "") : e.metadata.taskKey.replace(/:\d+:\d+$/, "");
export function calibratedConfidence(samples: Evidence[], now: Date) {
  const recent = samples.filter(e => now.getTime()-Date.parse(e.occurred_at)<=90*86400000);
  const diversity = new Set(recent.map(e => evidenceContext(e))).size;
  const good = recent.filter(e => e.response_quality!>=3).length;
  const poor = recent.filter(e => e.response_quality!<=1).length;
  if (!recent.length) return 0;
  if (diversity<3 || recent.length<3 || (good>0 && poor>0 && Math.min(good,poor)/recent.length>=0.25)) return 1;
  return recent.length>=6 && recent.filter(e=>e.transfer_success).length>=2 ? 3 : 2;
}
const diverse = (events: Evidence[]) => [...new Map(events.map(e => [evidenceContext(e),e])).values()].sort((a,b)=>a.occurred_at.localeCompare(b.occurred_at));
...
export function processEvidence(snapshot: Snapshot, incoming: Evidence[], now = new Date()): { patches: Patch[]; applied: string[] } {
  ...
  const valid = all.filter(eligible).filter(e=>now.getTime()-Date.parse(e.occurred_at)<=90*86400000); const patches: Patch[] = [];
  const changed = pending.filter(eligible).filter(e=>now.getTime()-Date.parse(e.occurred_at)<=90*86400000);
  for (const skill of new Set(changed.map(e => e.target_skill).filter(Boolean))) {
    ...
    const last = samples.slice(-3); const varied = diverse(samples).slice(-3); const latest = changed.filter(e => e.target_skill === skill).at(-1)!;
    // Each level needs three independent successes at/above that level. Old easy successes cannot raise it again.
    if (independent(latest) && varied.length === 3 && varied.every(e => e.response_quality! >= 3 && e.metadata.difficulty >= level)) level = Math.min(6, level + 1);
    ...
    confidence = calibratedConfidence(samples, now);
  ...
  for (const key of groups) {
    ...
    const successes = diverse(samples.filter(e => independent(e) && e.response_quality! >= 3)); const transfer = successes.filter(e => e.transfer_success).length;
    let state = old?.state ?? "unknown";
    if (latest.response_quality! >= 2) {
      const proposed = latest.support_level > 0 ? "supported" : successes.length >= 5 && transfer >= 2 && calibratedConfidence(samples.filter(independent),now)>=2 && samples.slice(-3).every(e=>independent(e)&&e.response_quality!>=3) ? "strong" : successes.length >= 3 ? (modality.endsWith("recognition") ? "recognized" : "independent") : "emerging";
    ...
    patches.push({ kind: "knowledge", key, values: { knowledge_item_id: kid, modality, state, confidence_level: calibratedConfidence(samples.filter(independent),now) || 1, review_need: latest.response_quality! <= 1 ? 3 : samples.slice(-2).length===2&&samples.slice(-2).every(e=>independent(e)&&e.response_quality!>=3) ? 0 : reviewNeed(state, old?.last_evidence_at ?? null, old?.review_need ?? 0, now), last_evidence_at: latest.occurred_at, trend: latest.transfer_success ? "improving" : "stable" }, evidence: samples.map(e => e.id) });
