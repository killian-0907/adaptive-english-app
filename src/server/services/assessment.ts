...
export async function submitAnswer(userId: string, input: unknown) {
  ...
  try {
    ...
    if (!answer.skip && resolveEvaluation(item.strategy,false,process.env.OPENAI_ENHANCED_EVALUATION==="true",!!process.env.OPENAI_API_KEY)==="AI_STRUCTURED") {
      try{
        evaluation = current.metadata.responseHash === hash && current.metadata.evaluation ? evaluationSchema.parse(current.metadata.evaluation) : await (async()=>{await requestSlot(userId,"evaluation",30,3600);return withAllowance(userId,"ai",()=>new OpenAIAssessmentProvider().evaluate(item,answer.text));})();
