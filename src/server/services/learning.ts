...
async function answer(userId:string,command:Extract<Command,{action:"answer"}>){
  ...
  try{
    ...
    const task=m.task;const decision=m.decision;let text=command.text;let voice=false;let browserVoice=false;let nativeVoice=false;
    if(command.voiceId){const receipt=checked(await db.from("voice_interactions").select("transcript,processing_status,provider").eq("user_id",userId).eq("activity_id",activity.id).eq("id",command.voiceId).eq("interaction_type","stt").single());if(receipt.processing_status!=="completed"||!receipt.transcript)throw new LearningError("Record again or type your response.");text=receipt.transcript;voice=true;nativeVoice=receipt.provider==="native_os";browserVoice=receipt.provider==="browser_native"||nativeVoice;}
    ...
    for(let retry=0;retry<4;retry++){
      ...
      const event=learningEvidence({snapshot:s,task,decision,scored,id,activityId:activity.id,knowledgeId:activity.target_knowledge_item_id,now,browserVoice,nativeVoice,voice,replay:m.replay??0,transcript:m.transcript??false,retries:Math.max(0,attempts.length-1),elapsedMs:command.elapsedMs,skip:command.skip,evaluationStrategy});
...
export async function learningCommand(userId:string,command:Command){
  ...
  else if((command.action==="browser_voice"||command.action==="native_voice")){await requireVoiceDelivery(userId);const voiceId=checked(await db.rpc(command.action==="native_voice"?"record_native_transcript":"record_browser_transcript",{p_user:userId,p_activity:command.activityId,p_attempt:command.attemptId,p_text:command.text}));return {voiceId,transcript:command.text};}
