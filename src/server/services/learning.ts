...
import { decide, makeTask, correctionText, supportText } from "@/domain/learning/engine";
...
function providerItem(task:Task,decision:Decision):Item{
  return {id:task.key,type:task.spoken?"spoken":task.audio?"listening":"written",skill:task.skill,modality:task.modality,difficulty:decision.difficulty,knowledge:task.topic,prompt:task.prompt,tts:task.audio?task.context:task.prompt,options:task.options,answer:task.answer,strategy:task.strategy,native:task.native,hint:task.explanation,expectedSkills:[task.skill],branching:"performance_and_coverage"};
}
...
async function nextActivity(userId:string,sessionId:string){
  for(let i=0;i<4;i++){
    ...
    const d=decide(s); const task=makeTask(d,s.history.filter(h=>h.session_id===sessionId).length);
...
type Summary={practiced:string[];worked:string[];needsPractice:string[];expressions:string[];next:string};
function summary(history:History[]):Summary{
  const done=history.filter(h=>h.status==="completed");
  return {practiced:[...new Set(done.map(h=>h.metadata.decision.objective))],worked:[...new Set(done.filter(h=>(h.metadata.quality??0)>=3&&!h.metadata.support).map(h=>h.metadata.decision.objective))],needsPractice:[...new Set(done.filter(h=>h.metadata.quality!==null&&((h.metadata.quality??4)<3||h.metadata.support)).map(h=>h.metadata.decision.objective))],expressions:[...new Set(done.map(h=>h.metadata.task.model))].slice(-2),next:done.at(-1)?.metadata.decision.returnRule??"Start with a short, useful everyday exchange."};
}
export type LearningView={kind:"start"|"assessment_required"|"activity"|"summary";sessionId?:string;activityId?:string;objective?:string;method?:string;prompt?:string;options?:string[];spoken?:boolean;audio?:boolean;support?:number;supportText?:string;nativeHelp?:string;transcript?:string;correction?:string;explanation?:string;speed?:number;words?:number;preparation?:number;frame?:string;feedbackDue?:boolean;summary?:Summary;savedVoice?:{id:string;transcript:string|null}|null};
export async function learningView(userId:string,endedSession?:string):Promise<LearningView>{
  ...
  return {kind:"activity",sessionId:session.id,activityId:id,objective:d.objective,method:d.method.replaceAll("_"," "),prompt:task.prompt,options:task.options,spoken:task.spoken,audio:task.audio,support,supportText:supportText(task,support,s.language),nativeHelp:d.exposure<=2||support>=5?task.native[s.language]??task.native.en:undefined,transcript:m.transcript?task.context:undefined,correction:latest?.metadata.correction,explanation:d.method==="grammar_explanation"?task.explanation:undefined,speed:d.listening.speed,words:d.speaking.words,preparation:d.speaking.preparationSeconds,frame:d.speaking.frame&&support>=4?`${task.model.split(" ")[0]} …`:undefined,feedbackDue:d.feedbackDue,savedVoice:receipts[0]??null};
}
async function answer(userId:string,command:Extract<Command,{action:"answer"}>){
  ...
  try{
    ...
    const task=m.task;const decision=m.decision;let text=command.text;let voice=false;
    if(command.voiceId){const receipt=checked(await db.from("voice_interactions").select("transcript,processing_status").eq("user_id",userId).eq("activity_id",activity.id).eq("id",command.voiceId).eq("interaction_type","stt").single());if(receipt.processing_status!=="completed"||!receipt.transcript)throw new LearningError("Record again or type your response.");text=receipt.transcript;voice=true;}
    ...
    let evaluation=null;
    if(!command.skip&&task.strategy==="evaluator"){
    ...
    const {quality,confidence,errors,support,skill,modality}=scoreLearning(task,text,evaluation,voice,m.transcript??false,m.support??0,command.skip);
    ...
    for(let retry=0;retry<4;retry++){
      ...
      const event:Evidence=evidenceSchema.parse({id,session_id:activity.session_id,activity_id:activity.id,target_skill:quality===null?null:skill,knowledge_item_id:activity.target_knowledge_item_id,modality,source:task.strategy==="exact"?"deterministic":"evaluator",evidence_kind:"learning_performance",result:quality===null?"neutral":quality>=3?"success":quality>=2?"partial":"failure",response_quality:quality,support_level:support,evaluator_confidence_level:confidence,transfer_success:decision.purpose==="transfer"&&quality!==null&&!s.evidence.some(e=>e.metadata.taskKey===task.key&&e.transfer_success!==null)?quality>=3:null,voice_uncertainty:false,response_time_ms:command.elapsedMs,dedupe_key:`${activity.id}:learning:performance`,occurred_at:now,processor_status:"pending",metadata:{difficulty:decision.difficulty,method:decision.method,topic:task.topic,taskKey:task.key,errors,sessionState:s.state,misunderstood:errors.includes("task_misunderstanding"),firstListen:task.audio&&m.replay===1&&!m.transcript,retries:Math.max(0,attempts.length-1),timed:false}});
