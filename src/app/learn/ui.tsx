...
export function LearningClient({initial}:{initial:LearningView}){const t=useT();
  ...
  async function action(data?:object){setBusy(true);setError("");try{setView(await (await api(data)).json());}catch(e){setError(friendlyError(e));}finally{setBusy(false);}}
  return <main className="assessment-shell"><header><Link href="/">Adaptive English</Link><span>{t("ui.074")}</span></header>
