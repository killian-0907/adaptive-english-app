...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    deletionSchema.parse(input);await deleteAccount(user.id);await(await createServerUserSupabaseClient()).auth.signOut();return NextResponse.json({deleted:true},{headers:{"Cache-Control":"no-store"}});
