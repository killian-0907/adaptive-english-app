...
export default async function LoginPage({ searchParams }: Props) {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-lg flex-col justify-center gap-8 p-8">
      <h1 className="text-2xl font-semibold">Authentication test</h1>
      ...
      <form action={signInAction} className="flex flex-col gap-3">
        ...
        <input className="border p-2" name="password" type="password" autoComplete="current-password" required minLength={8} placeholder="Password" />
        <button className="border p-2" type="submit">Sign in</button>
      </form>
      <form action={signUpAction} className="flex flex-col gap-3">
        <h2 className="font-medium">Create test account</h2>
        <input className="border p-2" name="email" type="email" autoComplete="email" required placeholder="Email" />
