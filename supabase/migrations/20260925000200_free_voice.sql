...
create function public.record_browser_transcript(p_user uuid,p_activity uuid,p_attempt uuid,p_text text)
...
begin
  ...
  select id into receipt from public.voice_interactions where id=p_attempt and user_id=p_user and activity_id=p_activity and provider='browser_native';
  ...
  values(p_attempt,p_user,a.session_id,a.id,n,'stt','browser_native','browser-managed-en-US',trim(p_text),'client_confirmed_unverified','completed',true,now()) returning id into receipt;
  return receipt;
end $$;
