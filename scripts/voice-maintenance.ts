...
export async function maintainVoice(db:DB){
  ...
  for(const job of jobs.data){try{await removeUserObjects(db,job.user_id);if(!job.completed_at){const r=await db.auth.admin.deleteUser(job.user_id);if(r.error&&r.error.status!==404)throw new Error("Identity unavailable");await db.from("account_deletion_jobs").update({completed_at:new Date(now).toISOString()}).eq("user_id",job.user_id);}else if(now-Date.parse(job.completed_at)>7*86400000){await db.from("account_deletion_jobs").delete().eq("user_id",job.user_id);}}catch{failed++;}}
