...
Web typecheck, lint, unit tests and production build passed for the fix. Lint retains the existing warning. These checks do not replace the remaining Phase 18 database/full-browser verification or physical retest. The first deployed fix was retested by the user and **failed**: another practice answer submitted, but feedback still did not appear visibly before continuing. Automated viewport success did not establish physical-device success.

### Reopened feedback bug: explicit acknowledgment

The first fix relied on scrolling feedback within the next activity. The replacement displays a dedicated response-feedback screen after a successful answer, with the server correction and an explicit Continue button. The next activity is not mounted until Continue is tapped. Continuing only reveals the already-saved server decision; it makes no additional answer request and adds no client-side scoring. If a successful response lacks correction text, the screen truthfully says only that the response was saved. English, Chinese and Spanish labels are included.

Regression coverage checks feedback in the mobile viewport, focus, absence of the next prompt/form before acknowledgment, and exactly one answer request. Existing voice and draft tests now acknowledge feedback explicitly. Verification of the replacement: typecheck passed; lint passed with the existing PostCSS warning; 215 unit tests passed; two targeted learning browser tests and seven affected voice/draft browser tests passed; production build passed; secret scan found zero findings in 275 files and three existing mobile bundles. Browser tests used the local Web UI with disposable hosted fixtures. An initial overlapping run lost its dev server; the sequential rerun passed all seven affected tests. These results do not claim a full database/browser suite run or physical background-recording success. Physical iPhone retest of this replacement remains pending; the bug is not marked passed.
