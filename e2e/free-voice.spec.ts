...
test("free A: browser listening and spoken transcript persist cautious evidence and continue",async({page})=>{
  ...
  try{const before=await current(page);expect(before.scenario?.family).toBe("restaurant");await play(page);await page.getByRole("button",{name:"Record response",exact:true}).click();await expect(page.getByRole("button",{name:"Confirm recognized text",exact:true})).toBeEnabled();await page.getByRole("button",{name:"Confirm recognized text",exact:true}).click();await expect(page.getByText("Recorded response ready.",{exact:true})).toBeVisible();await page.getByRole("button",{name:"Check response",exact:true}).click();await page.getByRole("button",{name:"Continue",exact:true}).click();await expect.poll(async()=>(await current(page)).scenario?.turn).toBe(1);
...
test("free B: unsupported recognition keeps typed fallback usable",async({page})=>{
  await speechMock(page,false);const f=await fixture(page);try{await expect(page.getByText(/Speech recognition is unavailable here/)).toBeVisible();await expect(page.getByRole("button",{name:"Record response",exact:true})).toBeDisabled();const before=await current(page);await page.getByRole("button",{name:"Read instead",exact:true}).click();await page.getByLabel("Your response",{exact:true}).fill("Water please");await page.getByRole("button",{name:"Check response",exact:true}).click();await page.getByRole("button",{name:"Continue",exact:true}).click();await expect.poll(async()=>(await current(page)).activityId).not.toBe(before.activityId);const events=await f.db.from("evidence_events").select("modality,support_level").eq("activity_id",before.activityId).eq("evidence_kind","learning_performance");expect(events.data![0]).toMatchObject({modality:"written_production",support_level:5});}finally{await f.cleanup();}
});
...
test("free E: first-listen evidence stays separate from revealed-text success",async({page})=>{
  await speechMock(page);const f=await fixture(page,"listening");try{
    const first=await current(page);await play(page);await page.getByLabel("Water, please.",{exact:true}).check();await page.getByRole("button",{name:"Check response",exact:true}).click();await page.getByRole("button",{name:"Continue",exact:true}).click();await expect.poll(async()=>(await current(page)).activityId).not.toBe(first.activityId);
    const second=await current(page);await page.getByRole("button",{name:"Read instead",exact:true}).click();await page.getByLabel("Still water, please.",{exact:true}).check();await page.getByRole("button",{name:"Check response",exact:true}).click();await page.getByRole("button",{name:"Continue",exact:true}).click();await expect.poll(async()=>(await current(page)).activityId).not.toBe(second.activityId);
...
test("free F: backgrounding stops native capture and playback without saving evidence",async({page})=>{
  ...
  const f=await fixture(page);try{
    ...
    const before=await current(page);await page.getByRole('button',{name:'Read instead',exact:true}).click();await page.getByLabel('Your response',{exact:true}).fill('Water please');await page.getByRole('button',{name:'Check response',exact:true}).click();await page.getByRole('button',{name:'Continue',exact:true}).click();await expect.poll(async()=>(await current(page)).activityId).not.toBe(before.activityId);
  }finally{await f.cleanup();}
});
